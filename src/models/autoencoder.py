"""
Dense Autoencoder para Detecção de Anomalias
"""
...
class DenseAutoencoderDetector(UnsupervisedDetector):
    """Detector baseado em Dense Autoencoder"""
    
    def __init__(self, seq_length=10, latent_dim=4, epochs=10, 
                 batch_size=32, contamination=0.05):
        super().__init__(name="Dense Autoencoder", contamination=contamination)
    ...
    def _build_model(self):
        """Constrói Dense Autoencoder"""
        input_layer = layers.Input(shape=(self.seq_length, 1))
        
        # Encoder
        encoder = layers.Flatten()(input_layer)
        encoder = layers.Dense(8, activation='relu')(encoder)
        encoder = layers.Dense(self.latent_dim, activation='relu', name='latent')(encoder)
        
        # Decoder
        decoder = layers.Dense(8, activation='relu')(encoder)
        decoder = layers.Dense(self.seq_length, activation='sigmoid')(decoder)
        decoder = layers.Reshape((self.seq_length, 1))(decoder)
    ...
    def _create_sequences(self, data):
        """Cria sequências para o autoencoder"""
        sequences = []
